...
pub trait FromFFIVector<T> {
    fn from_ffi_vector(v: T) -> Self;
}

impl FromFFIVector<ffi::Vector2> for Vector2<f32> {
    fn from_ffi_vector(v: ffi::Vector2) -> Self {
        Vector2::new(v.x, v.y)
    }
}

impl FromFFIVector<ffi::Vector3> for Vector3<f32> {
    fn from_ffi_vector(v: ffi::Vector3) -> Self {
        Vector2::new(v.x, v.y, v.z)
    }
}

pub trait ToFFIVector<T> {
    fn to_ffi_vector(&self) -> T;
}

impl ToFFIVector<ffi::Vector2> for Vector2<f32> {
    fn to_ffi_vector(&self) -> ffi::Vector2 {
        ffi::Vector2 {
            x: self.x,
            y: self.y,
        }
...
impl ToFFIVector<ffi::Vector3> for Vector3<f32> {
    fn to_ffi_vector(&self) -> ffi::Vector3 {
        ffi::Vector3 {
            x: self.x,
            y: self.y,
            z: self.z,
...
pub trait ToFFIMatrix<T> {
    fn to_ffi_matrix(&self) -> T;
}

impl ToFFIMatrix<ffi::Matrix> for nalgebra::Matrix4<f32> {
    fn to_ffi_matrix(&self) -> ffi::Matrix {
        ffi::Matrix {
            m0: self.m00,
            m1: self.m01,
            m2: self.m02,
...
pub trait FromFFIMatrix<T> {
    fn from_ffi_matrix(m: T) -> Self;
}

impl FromFFIMatrix<ffi::Matrix> for nalgebra::Matrix4<f32> {
    fn from_ffi_matrix(m: ffi::Matrix) -> Self {
        nalgebra::Matrix4::new(
            m.m0, m.m1, m.m2, m.m3, m.m10, m.m11, m.m12, m.m13, m.m20, m.m21, m.m22, m.m23,
...
// // impl<T> From<ffi::Vector2> for cgmath::Vector2<T>
// // where
// //     T: Float,
// // {
// //     fn from(v: ffi::Vector2) -> cgmath::Vector2<T> {
// //         cgmath::Vector2::new(v.x, v.y)
// //     }
// // }

// // impl Into<ffi::Vector2> for cgmath::Vector2 {
